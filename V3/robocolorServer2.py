...
def send_to_target(target_name, message):
    with ClientListLock:
        for client in ClientList:
            if client.state == STATE_CLIENT_RUNNING and client.name == target_name:
        ...
        while True:
            ...
            for line in lines[:-1]:
                ...
                if ( client_msg_tokens[0] == MSG_REGISTER ):
                    ...
                    if ' from ' in decoded_msg:
                        from_parts = decoded_msg.split( ' from ', 1 )
                        if len( from_parts ) > 1:
                            after_from = from_parts[1]
                            if ' to ' in after_from and ':' in after_from:
                                msg_from_part, after_to = after_from.split( ' to ', 1 )
                                msg_from = msg_from_part.strip()
                                msg_to_part, msg_payload = after_to.split( ':', 1 )
                                msg_to = msg_to_part.strip()
                                msg_payload = msg_payload.strip()
                            else:
                                msg_from = after_from.split()[0]
                                msg_payload = ' '.join( client_msg_tokens[3:] )
                    elif len( client_msg_tokens ) > 2:
                        msg_from = client_msg_tokens[2]
                        msg_payload = ' '.join( client_msg_tokens[3:] )
                    ...
                    forwarded_msg = 'Forwarding ' + msg_type + ' from ' + msg_from
                    if msg_to:
                        forwarded_msg += ' to ' + msg_to
                    if msg_payload:
                        forwarded_msg += ': ' + msg_payload if msg_to else ' ' + msg_payload
                    forwarded = False
                    if msg_to:
                        forwarded = send_to_target( msg_to, forwarded_msg )
                        if forwarded:
                            print( '[server client %s]: forwarding message [%s] to [%s]' % ( self.name, forwarded_msg, msg_to ))
                    if not forwarded:
                        with ClientListLock:
